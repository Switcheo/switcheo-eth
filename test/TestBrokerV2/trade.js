...
contract('Test trade', async (accounts) => {
    ...
    contract('test', async () => {
        it('tests', async () => {
            const result = await broker.test([7, 21, 35, 78, 1])
            const { v1, v2 } = result.receipt.logs[0].args
            console.log('result', v1.toString(), v2.toString())
        })
    })

    contract('best-case optimised batched trades', async () => {
        it('test gas cost', async () => {
            const batchSize = 5
